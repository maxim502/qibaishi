...
- (void)chooseBeginner
{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:GAME_TITLE message:@"谁先来 ?" delegate:self cancelButtonTitle:@"电脑" otherButtonTitles:@"我", nil];
    alert.tag = 666;
    [alert show];
    [alert release];
}
...
- (void)checkEndGameStatus
{
    if(!gamePaused)
    {
        ...
        if(gameEnded)
        {
            ...
            if(scoreX > scoreO) {
                title = @"恭喜 !";
                message = [NSString stringWithFormat:@"你赢了 %d 比 %d  !", scoreX, scoreO];
            } else if(scoreX < scoreO) {
                title = @"惜败 !";
                message = [NSString stringWithFormat:@"你输了 %d 比 %d ...", scoreX, scoreO];
            } else {
                title = @"平局 !";
                message = [NSString stringWithFormat:@"平局 ! %d 比 %d !", scoreX, scoreO];
            }
            
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title message:message delegate:self cancelButtonTitle:@"新游戏" otherButtonTitles:nil];
            alert.tag = 777;
            [alert show];
            [alert release];
        }
...
- (IBAction)caseCliked:(id)sender
{
    ...
    if(mapArray[x][y]==MAP_BLANK)
    {
        if(yourTurn)
        {
            ...
            {
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:GAME_TITLE message:@"你隔壁没有黑色的棋子 !" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
                [alert show];
                [alert release];
            }
        }
    ...
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:GAME_TITLE message:@"这里已经有一块了 !" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alert show];
        [alert release];
    }
}
